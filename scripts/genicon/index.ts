...
const iconComponentPath = path.resolve(root, 'src/icon/icons')
...
const buildIconComponent = async (svgData: IconData[]) => {
  ...
  await fs.emptyDir(path.resolve(root, 'src/icon/icons'))
...
const buildIndex = async (svgData: IconData[]) => {
  ...
  try {
    await fs.outputFile(
      path.resolve(root, 'src/icon/icons/sheep-vue-icon.ts'),
      getSheepVueIcon({ imports, components })
    )
    console.log('build sheep-vue-icon.ts success')

    await fs.outputFile(
      path.resolve(root, 'src/icon/icons/index.ts'),
      getIndex(exports)
    )
