...
export const RightCircleIcon = (props: IconProps) => {
  ...
  const sizeStyle = props.size
    ? isNumber(props.size)
      ...
      : props.size
    : undefined
  let classes = `${prefixCls} ${prefixCls}-right-circle`

  if (props.spin) classes += ` ${prefixCls}-spin`

  return (
    <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" class={classes} stroke-width={strokeWidth} style={sizeStyle} stroke-linecap={strokeLinecap} stroke-linejoin={strokeLinejoin}><circle cx="24" cy="24" r="18"/><path d="M19.485 15.515 27.971 24l-8.486 8.485"/></svg>
  )
}