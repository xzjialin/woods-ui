...
export const ObliqueLineIcon = (props: IconProps) => {
  const {
    strokeWidth = 4,
    strokeLinecap = 'butt',
  ...
  const sizeStyle = props.size
    ? isNumber(props.size)
      ...
      : props.size
    : undefined
  let classes = `${prefixCls} ${prefixCls}-oblique-line`

  if (props.spin) classes += ` ${prefixCls}-spin`

  return (
    <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" class={classes} stroke-width={strokeWidth} style={sizeStyle} stroke-linecap={strokeLinecap} stroke-linejoin={strokeLinejoin}><path d="M29.506 6.502 18.493 41.498"/></svg>
  )
}