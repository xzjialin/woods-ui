...
export const MinusIcon = (props: IconProps) => {
  ...
  const sizeStyle = props.size
    ? isNumber(props.size)
      ...
      : props.size
    : undefined
  let classes = `${prefixCls} ${prefixCls}-minus`

  if (props.spin) classes += ` ${prefixCls}-spin`

  return (
    <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" class={classes} stroke-width={strokeWidth} style={sizeStyle} stroke-linecap={strokeLinecap} stroke-linejoin={strokeLinejoin}><path d="M5 24h38"/></svg>
  )
}