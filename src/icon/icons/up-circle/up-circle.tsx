...
export const UpCircleIcon = (props: IconProps) => {
  ...
  const sizeStyle = props.size
    ? isNumber(props.size)
      ...
      : props.size
    : undefined
  let classes = `${prefixCls} ${prefixCls}-up-circle`

  if (props.spin) classes += ` ${prefixCls}-spin`

  return (
    <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" class={classes} stroke-width={strokeWidth} style={sizeStyle} stroke-linecap={strokeLinecap} stroke-linejoin={strokeLinejoin}><circle cx="24" cy="24" r="18"/><path d="M15.516 28.485 24 20l8.485 8.485"/></svg>
  )
}